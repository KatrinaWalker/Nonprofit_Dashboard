...
# cleaning and setting paths

rm(list = ls())

library(RCurl)

state.codes <- "C:/ALFONSO/WORK/GSE/14D003/state codes.csv" ## alter theese depending on the machine

saving.folder <- "C:/ALFONSO/WORK/GSE/14D003/csv/" ## alter theese depending on the machine
  ...
  for (y in 1:3) {
    for (p in 1:3) {
      for (s in 1:3) {
        ...
        # composing the filename and path
